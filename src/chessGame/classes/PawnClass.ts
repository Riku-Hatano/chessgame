...
class Pawn extends Piece {
    ...
    place(rank: number, file: number) {
        ...
        if(this.color == "white") {
            ...
            if(document.getElementById((file + 1).toString() + (rank - 1).toString())?.innerHTML[0] == "b") { availablePlaces.push((file + 1).toString() + (rank - 1).toString()) };
            if(this.isMoved == false && document.getElementById(file.toString() + (rank - 2).toString())?.innerHTML[0] == undefined && document.getElementById(file.toString() + (rank - 1).toString())?.innerHTML[0] == undefined) { availablePlaces.push(file.toString() + (rank - 2).toString())};
        } else {
